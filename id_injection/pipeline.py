...
def load_final_id_embeds(file_path):
    return torch.load(file_path)


def load_prompt_and_embeds(subdir):
    ...
    final_id_embeds = load_final_id_embeds(embeds_path)
    
    return prompt, final_id_embeds
...
class IdentityInjectionPipeline(StableDiffusionXLControlNetPipeline):
    ...
    def __init__(
        ...
        controlnet: Optional[Union[ControlNetModel, List[ControlNetModel], Tuple[ControlNetModel], MultiControlNetModel]],
    ...
    def __call__(
        ...
        **kwargs,
        ...
        controlnet = None if self.controlnet is None else (
        ...
        ) if controlnet is not None else False
        ...
        ref_ids = [pt[2] for pt in prompt[1]]

        # concat_prompts = global_prompt + region_prompts
        
        # IdentityStory: remove trigger words in region prompts to perform text encoding
        cleaned_region_prompts = [prompt.replace(f" {concept_models.trigger_word}", "") for prompt in region_prompts]
        concat_prompts = global_prompt + cleaned_region_prompts
        ...
        if stage==2:
            ...
            # use photomaker to extract ID embeddings
            # image_prompt_image_emb_list = []
            # for img_path in ref_images:
            #     img = load_image(img_path)
            #     prompt_image_emb = concept_models.get_final_id_embeds([img])
            #     image_prompt_image_emb_list.append(prompt_image_emb)

            # IdentityStory: directly use saved ID
            image_prompt_image_emb_list = []
            for id_dir in ref_ids:
                _, prompt_image_emb = load_prompt_and_embeds(id_dir)
                image_prompt_image_emb_list.append(prompt_image_emb)
        ...
        if t2i_image is not None:
            t2i_image = self.prepare_image(
                ...
                num_images_per_prompt=num_images_per_prompt,
                device=device,
                dtype=controlnet.dtype if controlnet is not None else self.unet.dtype,
                do_classifier_free_guidance=self.do_classifier_free_guidance,
                guess_mode=guess_mode,
            )
        ...
        is_controlnet_compiled = self.controlnet is not None and is_compiled_module(self.controlnet)
        is_torch_higher_equal_2_1 = is_torch_version(">=", "2.1")
        # hyper-parameters
        scale_range = np.linspace(1, 0.5, len(self.scheduler.timesteps))

        # IdentityStory: cache noisy prediction
        if stage == 1:
            self.cached_noise_pred = []
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # IdentityStory: cache noisy prediction
                if stage == 1:
                    self.cached_noise_pred.append(noise_pred)

                if (i + 1) >= args.start_timestep and stage == 2:
                    assert len(self.cached_noise_pred) == len(timesteps)

                    # IdentityStory: progressive masks
                    prog_mask_list = get_progressive_masks(mask_list, args, i + 1, args.start_timestep, len(timesteps), args.max_dilation_kernel)

                    region_mask = self.get_region_mask(prog_mask_list, noise_pred.shape[2], noise_pred.shape[3])
                    # edit_noise = torch.concat([noise_pred[1:2], noise_pred[3:4]], dim=0)
                    # edit_noise = torch.concat([noise_pred[0:1], noise_pred[2:3]], dim=0)  # IdentityStory: use cached noisy pred
                    edit_noise = torch.concat([self.cached_noise_pred[i][0:1], self.cached_noise_pred[i][2:3]], dim=0) # IdentityStory: use cached noisy pred (the real version)
                    new_noise_pred = edit_noise * (1 - region_mask.to(edit_noise.device))
                    ...
                    for region_prompt_embeds, region_add_text_embeds, region_add_time_ids, concept_mask, region_prompt, region_prompt_image_emb in zip(region_prompt_embeds_list, region_add_text_embeds_list, add_time_ids_list, prog_mask_list, region_prompts, image_prompt_image_emb_list):
                        if concept_mask is not None:
                            ...
                            # IdentityStory: change to be more normal, but it has the same effect
                            region_latent_model_input = torch.concat([latent_model_input[1:2], latent_model_input[3:4]], dim=0).to(concept_models._execution_device)

                            region_added_cond_kwargs = {"text_embeds": region_add_text_embeds,
                                                        "time_ids": region_add_time_ids}

                            if image is not None and controlnet is not None:
                            ...
                            # IdentityStory: use photomaker to fuse text embeddings and ID embeddings
                            class_tokens_mask = concept_models.get_class_tokens_mask(region_prompt)
                            region_prompt_embeds[1] = concept_models.id_encoder.update_prompt_embeds(
                                region_prompt_embeds[1].unsqueeze(0), 
                                region_prompt_image_emb, 
                                class_tokens_mask
                            )
                            region_encoder_hidden_states = region_prompt_embeds.clone()
                            ...
                            new_noise_pred += region_noise_pred * concept_mask.to(concept_models._execution_device)
    ...
    def get_region_mask(self, mask_list, feat_height, feat_width):
        ...
        for mask in mask_list:
            if mask is not None:
                ...
                exclusive_mask += mask
        exclusive_mask = torch.clamp(exclusive_mask, 0, 1)
...
def retain_largest_component(mask):
...
def get_progressive_masks(mask_list, args, i, i_start, i_end, max_dilation_kernel):

    prog_mask_list = [None] * len(mask_list)

    cover_ratio = (i - i_start) / (i_end - i_start)  # from 0 to 1

    indexed_masks = [(j, mask, calculate_mask_area(mask)) for j, mask in enumerate(mask_list)]
    ...
    for original_index, mask, _ in sorted_masks:
        ...
        # IdentityStory: dilated mask
        current_kernel = int(max_dilation_kernel * cover_ratio)
        dilated_mask = dilate_mask(mask, current_kernel)

        non_overlapping_mask = dilated_mask & ~covered_regions
        prog_mask_list[original_index] = non_overlapping_mask.float()
        covered_regions |= non_overlapping_mask

    # save masks
    if args.save_info:
        save_mask_dir = os.path.join(args.save_info_dir, 'prog_masks_dilated')
        os.makedirs(save_mask_dir, exist_ok=True)
        for idx, prog_mask in enumerate(prog_mask_list):
            file_path = os.path.join(save_mask_dir, f"prog_mask_{idx}_step{i}.png")
            save_mask_tensor_as_image(prog_mask, file_path)

    return prog_mask_list
